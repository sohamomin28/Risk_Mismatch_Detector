...
Ingestion: Loads clients.csv and converts dollar allocations into physical ETF shares (SPY/AGG).

Analysis: Performs vectorized calculations to monitor daily drift and volatility for the entire client book.

Modeling: Trains a predictive AI model to link drift and low performance to potential client attrition.

Compliance & Fix: Runs suitability checks and calculates the "Execution Gap" to output trade instructions.

Reporting: Exports a final_bank_report.csv for management oversight.
